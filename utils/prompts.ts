...
export function getIndexNamePrompt(jsonData: any[], question: string) {
    let promptString =
...
export function getPriorityDecisionPrompt(kpiJsonData: Record<string, number>): string {
    let promptString = `
    Your job is now to act as my business analyst. I'm going to provide you with a set of current
    metrics in the business, you are going to evaluate the metrics given with the objective of finding
    constraints and then identify the #1 focus point based on the order of priority in the list I provide.
...
export function getAnswerPrompt(context: any[], question: string, priorityConstraint: string, actionSteps: string): string {
    ...
    let promptString = `
    you are a hyper emotionally intelligent ai. Your client is a business owner.
    Your ONE job is to keep the client focused on their highest priority constraint:
    ${priorityConstraint}
    ...
    Clients often ask questions unrelated to the highest priority constraint.
    In your response to the question, after answering their main question with emotional intelligence,
    ALWAYS make sure to mention the action steps for the highest priority constraint:
    
    ${actionSteps}

    If the client says or asks "${question}", 
    then what should you respond in order to be in full alignment with the instructions?`
